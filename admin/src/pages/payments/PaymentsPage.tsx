      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400 ml-auto">{payments.data?.total ?? 0} payments</span>
          </div>
        </CardHeader>
        <CardBody className="p-0">
          {payments.isLoading ? (
            ...
            <p className="px-6 py-10 text-center text-sm text-gray-500 dark:text-gray-400">No payments found.</p>
          ) : (
            <>
              {/* Mobile cards */}
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {payments.data.items.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                        <td className="px-6 py-3 font-medium text-gray-700 dark:text-gray-300">{p.invoice?.invoiceNumber ?? '—'}</td>
                        <td className="px-6 py-3 font-medium dark:text-gray-100">{formatCurrency(p.amount, p.currency)}</td>
                        <td className="px-6 py-3 text-gray-600 dark:text-gray-400 capitalize">{p.paymentMethodType}</td>

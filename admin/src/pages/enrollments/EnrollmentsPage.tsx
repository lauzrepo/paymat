...
export function EnrollmentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <Card>
          <CardHeader><h2 className="text-base font-semibold text-gray-900">New Enrollment</h2></CardHeader>
          <CardBody>
            <form onSubmit={handleEnroll} className="grid grid-cols-3 gap-4">
              <div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500 ml-auto">{enrollments.data?.total ?? 0} enrollments</span>
          </div>
        </CardHeader>
        <CardBody className="p-0">
          {enrollments.isLoading ? (
            ...
            <p className="px-6 py-10 text-center text-sm text-gray-500">No enrollments found.</p>
          ) : (
                ...
                {enrollments.data.items.map((e) => (
                  <tr key={e.id} className="hover:bg-gray-50">
                    <td className="px-6 py-3 font-medium">
                      <Link to={`/contacts/${e.contactId}`} className="text-indigo-600 hover:underline">
                        {e.contact?.firstName} {e.contact?.lastName}
                      </Link>
                    </td>
                    <td className="px-6 py-3">{e.program?.name}</td>
                    <td className="px-6 py-3">{formatCurrency(e.program?.price ?? 0)}</td>
                    ...
                    <td className="px-6 py-3">
                      <div className="flex gap-1">
                        ...
                        {e.status !== 'cancelled' && (
                          <Button variant="ghost" size="sm" onClick={() => unenroll.mutate({ id: e.id })}>
                            Unenroll
                          </Button>
                        )}
                        <Button
